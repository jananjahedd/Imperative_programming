...
def init():
    """
    Reads a KB from standard input
    """
    kb = []

    kb_input = input("Enter the KB in the format [[clause1],[clause2],...]: ")
    kb_list = eval(kb_input)

    for clause in kb_list:
        kb.append(Clause(','.join(clause)))
...
def recursive_print_proof(idx, clause_set):
    clause = clause_set[idx]
    resolvents = []

    # Find the resolvent clauses that were used to derive the current clause
    for i in range(len(clause_set)):
        if i != idx:
            if can_resolve(clause, clause_set[i]):
                resolvents.append(clause_set[i])

    # Recursively print the resolvents and their derivation
    for resolvent in resolvents:
        recursive_print_proof(find_index_of_clause(resolvent, clause_set), clause_set)

    # Print the current clause and its derivation
    print(f"{clause.print_clause()} is inferred from ", end='')
    for resolvent in resolvents[:-1]:
        print(f"{resolvent.print_clause()} and ", end='')
    print(f"{resolvents[-1].print_clause()}.")
